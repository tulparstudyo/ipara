...
!function ($) {
    ...
    jQuery.fn.CardJs = function (methodOrOptions) {
        return methods[methodOrOptions] ? methods[methodOrOptions].apply(this, Array.prototype.slice.call(arguments, 1)) : "object" != typeof methodOrOptions && methodOrOptions ? void jQuery.error("Method " + methodOrOptions + " does not exist on jQuery.CardJs") : methods.init.apply(this, arguments)
    }
    ...
    if (isNumber || isDelete || isBackspace) {
        e.preventDefault();
        var rawText = jQuery(element).val(), numbersOnly = CardJs.numbersOnlyString(rawText),
    ...
    if (this.EXPIRY_USE_DROPDOWNS) {
        ...
        for (var expiryYearNew = jQuery("<select><option value='any' selected='' hidden=''>YY</option></select>"), currentYear = parseInt((new Date).getFullYear().toString().substring(2, 4)), i = 0; i < CardJs.EXPIRY_NUMBER_OF_YEARS; i++) expiryYearNew.append("<option value='" + currentYear + "'>" + currentYear + "</option>"), currentYear = (currentYear + 1) % 100;
